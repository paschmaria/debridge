		        				...
		        				<div class="row">
			        				...
			        				@if(in_array($user->id, $following_ids))
			        					{{-- <form method="post" action="{{ route('unfollow', $user->reference) }}"> --}}
			        						<button class="btn unfollow btn-sm f-14 waves-light waves-effect btn-brand m-t-40 m-b-50" data-email="{{$user->reference}}" data-id="{{$user->id}}" data-fname="{{$user->full_name()}}" ></span>Unfollow</button>
			        					{{-- </form> --}}
			        				@else
